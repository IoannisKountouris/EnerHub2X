...
Display Slack,SellUpSet,SellDownSet,BuyUpSet,BuyDownSet;
*$exit
...
FuelusetotalLimProfiles(tech,time)$sum(time1,profile(time1,tech) and not (techdata(tech,'CapacityUP') or techdata(tech,'CapacityDown')))..
    Fuelusetotal(tech,time)$sum(time1,profile(time1,tech))
    =L=
...
* Results
ResultT('Operation_MW',tech,energy ,time)=Generation.l(tech,energy,time)-Fueluse.l(tech,energy,time);
...
ResultT('Volume_MWh',out(tech,energy),time)=Volume.l(tech,time);
* TODO: price from which area?
ResultT('Costs_kr',tech,energy,time)=
    + Fueluse.l(tech,energy,time)*sum(area,price(time,area,energy,'import'))
    - Generation.l(tech,energy,time)*sum(area,price(time,area,energy,'export'));
...
ResultT('Variable_OM_cost_kr',tech,'system_cost',time) = Fuelusetotal.l(tech,time)*techdata(tech,'VariableOmcost');
...
*Day ahead
ResultT_all('DayAhead','Operation_MW',tech,energy,time)=Generation.l(tech,energy,time)-Fueluse.l(tech,energy,time);
ResultT_all('DayAhead','Volume_MWh',out(tech,energy),time)=Volume.l(tech,time);
...
ResultT_all('Up','Volume_MWh',out(tech,energy),time)=Volume.l(tech,time);
...
ResultT_all('Down','Volume_MWh',out(tech,energy),time)=Volume.l(tech,time);
...
ResultC_all('DayAhead','Capacity_MW',tech,time) = Fuelusetotal.l(tech,time)*techdata(tech,'Fe');
